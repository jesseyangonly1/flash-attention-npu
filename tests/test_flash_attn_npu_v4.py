...
test_cases = [
    ...
    (torch.bfloat16, 2, 6, 2, 2, 1024, 128, 1, 128, False, "BSND", False),   # BSND GQA prefill
    (torch.bfloat16, 5, 4, 4, 1024, 1024, 128, 1, 128, True, "TND", False),   # TND causal GQA prefill
    (torch.float16, 7, 1, 1, 512, 512, 128, 1, 128, False, "TND", False),     # fp16
    (torch.bfloat16, 1, 1, 1, 1, 1024, 128, 1, 128, True, "TND", False),      # decode causal
    (torch.bfloat16, 1, 1, 1, 1, 1024, 128, 1, 128, False, "BSND", False),    # decode BSND
    (torch.bfloat16, 1, 1, 1, 1, 4096, 128, 1, 128, False, "TND", False),     # decode long kv (num_splits)
    (torch.bfloat16, 2, 6, 2, 16, 1024, 128, 1, 128, False, "TND", True),     # varied GQA
    (torch.bfloat16, 1, 64, 1, 2, 1024, 256, 1, 128, False, "TND", False),    # head_size=256
    (torch.bfloat16, 2, 1, 1, 16, 1024, 256, 1, 128, True, "TND", False),     # head_size=256 causal
]
...
test_cases = [
    ...
    (torch.bfloat16, 1, 1, 1, 512, 1024, 128, True),          # bf16 causal
    (torch.float16, 7, 5, 1, 512, 512, 128, True),             # fp16 GQA causal
    (torch.float16, 7, 5, 1, 777, 888, 192, False),            # fp16 non-causal odd head_size
    (torch.bfloat16, 7, 5, 1, 711, 8192, 111, True),           # bf16 causal long kv odd head_size
]
